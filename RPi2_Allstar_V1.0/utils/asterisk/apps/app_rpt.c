...
#include <termios.h>
#include <sys/sysinfo.h>			// KB4FXC 2014-09-28
...
static  char *tdesc = "Radio Repeater / Remote Base  version 0.325-BBB 03/10/2015";
...
static struct rpt
{
	...
	struct {
		...
		char *archivedir;
		int archiveaudio;	// KB4FXC 2014-09-06
		int authlevel;
		char *csstanzaname;
		char *skedstanzaname;
...
// CRITICAL!  KB4FXC 2014-09-27 ...NOTE: renamed sys_uptime to avoid collision with existing
// uptime variable.
...
static int uchameleon_pin_init(struct daq_entry_tag *t)
{
	...
	sys_uptime(&t->adcacqtime); /* Start ADC Acquisition */ 
	return -0;
}
...
static void *uchameleon_monitor_thread(void *this)
{
	...
	for(;;){
		...
		if(sys_uptime(&now) >= t->adcacqtime){
...
static char *cs_keywords[] = {"rptena","rptdis","apena","apdis","lnkena","lnkdis","totena","totdis","skena","skdis",
	...
	val = (char *) ast_variable_retrieve(cfg,this,"archiveaudio");		// KB4FXC 2014-09-05
	if (val) rpt_vars[n].p.archiveaudio = atoi(val); 			// KB4FXC 2014-09-05
	else rpt_vars[n].p.archiveaudio = 1;					// KB4FXC 2014-09-05
...
static int rpt_do_dump(int fd, int argc, char *argv[])
{
	...
	for(i = 0; i < nrpts; i++)
	{
		...
		{
			rpt_vars[i].disgorgetime = sys_uptime(NULL) + 10; /* Do it 10 seconds later */
...
static int function_cop(struct rpt *myrpt, char *param, char *digitbuf, int command_source, struct rpt_link *mylink)
{
	...
	switch(myatoi(argv[0])){
		...
		case 5: /* Disgorge variables to log for debug purposes */
			myrpt->disgorgetime = sys_uptime(NULL) + 10; /* Do it 10 seconds later */
			return DC_COMPLETE;
...
struct	ast_frame wf;
	...
	if ((!myrpt->inpadtest) && (c == myrpt->p.funcchar))
	{
		...
		sys_uptime(&myrpt->rem_dtmf_time);
		rpt_mutex_unlock(&myrpt->lock);
		return;
	} 
...
int	ret,res = 0,src;
	...
	sys_uptime(&myrpt->last_activity_time);
...
char tmpstr[300],lstr[MAXLINKLIST],lat[100],lon[100],elev[100];
	...
	*tele++ = 0;
	...
	myrpt->keyed = 0;
	myrpt->txkeyed = 0;
	sys_uptime(&myrpt->lastkeyedtime);
	myrpt->lastkeyedtime -= RPT_LOCKOUT_SECS;
	sys_uptime(&myrpt->lasttxkeyedtime);
	...
	while (ms >= 0)
	{
		...
		/* DEBUG Dump */
		if((myrpt->disgorgetime) && (sys_uptime(NULL) >= myrpt->disgorgetime)){
			struct rpt_link *zl;
			struct rpt_tele *zt;
		...
		if (totx && (!lasttx))
		{
			...
			if (myrpt->p.archivedir)
			{
				...
				if (myrpt->p.archiveaudio) {	// KB4FXC 2014-09-06
					time(&myt);
		...
		if (who == myrpt->rxchannel) /* if it was a read from rx */
		{
			...
			else if (f->frametype == AST_FRAME_CONTROL)
			{
				...
				if (f->subclass == AST_CONTROL_RADIO_KEY)
				{
					...
					if (myrpt->p.archivedir)
					{
						if (myrpt->p.archiveaudio && myrpt->p.duplex < 2)	// KB4FXC 2014-09-07
						{
	...
	for(;;)
	{
		...
		{ 
			...
			if (rv)
			{
				...
				if(sys_uptime(NULL) - rpt_vars[i].lastthreadrestarttime <= 5)
				{
				...
				rpt_vars[i].lastthreadrestarttime = sys_uptime(NULL);
			        pthread_attr_init(&attr);
	 		        pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
				ast_pthread_create(&rpt_vars[i].rpt_thread,&attr,rpt,(void *) &rpt_vars[i]);
				/* if (!rpt_vars[i].xlink) */
...
//	if(altp>0)ast_log(LOG_NOTICE,"altp=%s \n",altp);
	...
	*--tele = '/';
	...
	if (myrpt->p.archivedir)
	{
		...
		time(&myt);
		strftime(mydate,sizeof(mydate) - 1,"%Y%m%d%H%M%S", localtime(&myt));
		sprintf(mycmd,"mixmonitor start %s %s/%s/%s.wav49 a",chan->name, myrpt->p.archivedir,myrpt->name,mydate);
		if (myrpt->p.monminblocks)
		{
			blocksleft = diskavail(myrpt);
		...
		{ 
				    	...
				    	myrpt->p.remotetimeoutwarningfreq)))
			{
				if (myrpt->p.remotetimeoutwarningfreq)
				{
					...
					myrpt->p.remotetimeoutwarningfreq)
				    {
					sys_uptime(&last_timeout_warning);
