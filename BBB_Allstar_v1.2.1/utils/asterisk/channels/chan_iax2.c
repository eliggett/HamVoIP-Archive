...
static unsigned int iax2_datetime(const char *tz)
{
	...
	struct tm tm;
	unsigned int tmp;
	sys_uptime(&t);			// Should we do this here???  KB4FXC 2014-10-01
...
{
#define FORMAT2 "%-27.27s  %-15.15s  %-10.10s  %-11.11s  %-11.11s  %-7.7s  %-6.6s  %-6.6s  %s  %s  %9s\n"
#define FORMAT  "%-27.27s  %-15.15s  %-10.10s  %5.5d/%5.5d  %5.5d/%5.5d  %-5.5dms  %-4.4dms  %-4.4dms  %-6.6s  %s%s  %3s%s\n"
#define FORMATB "%-27.27s  %-15.15s  %-10.10s  %5.5d/%5.5d  %5.5d/%5.5d  [Native Bridged to ID=%5.5d]\n"
	int x;
	int numchans = 0;
...
static int ast_cli_netstats(struct mansession *s, int fd, int limit_fmt)
{
	...
	for (x = 0; x < ARRAY_LEN(iaxs); x++) {
		...
		if (iaxs[x]) {
			...
			iax_frame_subclass2str(iaxs[x]->last_iax_message & ~MARK_IAX_SUBCLASS_TX, last_message, sizeof(last_message));
			if (limit_fmt)
				fmt = "%-27.27s %4d %4d %4d %5d %3d %5d %4d %6d %4d %4d %5d %3d %5d %4d %6d %s%s %4s%s\n";
			else
